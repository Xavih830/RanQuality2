...
interface DataOutput {
    handleDataChange : (category : string, limit: string, date: string) => void
}
...
export default function DoubleSelection(props : DataOutput) {
    ...
    const handleChange = (e : SelectCustomEvent) => {
        ...
        const limite = Number(e.detail.value);

        if (isNaN(limite)){
            setDate(e.detail.value);
        } else {
            setLimit(e.detail.value);
        }

        if (category != undefined && limit != undefined && date != undefined){
            props.handleDataChange(category, limit, date);
